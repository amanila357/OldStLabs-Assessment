...
class EditBooking extends React.Component {
  ...
  handleName(e) {
    this.props.onChange('updName', e.target.value);
  }

  handleComments(e) {
    this.props.onChange('updComments', e.target.value);
  }

  handleID(e) {
    this.props.onChange('updID', e.target.value);
  }

  handleDate(e) {
    const date = new Date(e.target.value);
    if (date.getDay()) {
      this.props.onChange('updDate', e.target.value);
    }
    else {
      alert('Booking on a Sunday is not allowed')
      this.props.onChange('updDate', '');
    }
  }

  handleStartTime(e) {
    this.props.onChange('updStartTime', e.target.value);
  }

  handleEndTime(e) {
    this.props.onChange('updEndTime', e.target.value);
  }
  ...
  render() {
    const { name, comments, id, message, date, startTime, endTime } = this.props;
    const msg = {
      textAlign: 'center'
    };
    const table = {
    ...
    return (
      <div>
        <h2 style={msg}>Edit an Appointment</h2>
        <h3 style={msg}>{message}</h3>
        <table style={table}>
          <tbody>
            <tr>
              <td>Booking ID:</td><td><input name="id" value={id} onChange={this.handleID} /></td>
            </tr>
            <tr>
            ...
            <tr>
              <td>Date:</td><td><input type="date" name="date" value={date} onChange={this.handleDate} /></td>
            </tr>
            <tr>
